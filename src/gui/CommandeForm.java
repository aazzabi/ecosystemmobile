         ...
         listcmd=commandeService.getCmdClient(Utilisateur.current_user.getId());
          for (Commande a : listcmd) {
            ...
            public Component[] fetchComponents(int index, int amount) {
                if (index == 0) {
                    listcmd=commandeService.getAllCommandes();
                    amount = listcmd.size();
                ...
                for (Commande a : listcmd) {
                    ...
                    try {
                        Image img = Image.createImage("file:/C:/wamp/www/ecosystemweb/web/uploads/Annonce/photo/cmd.jpg").fill(300, 300);
Button j = new Button("");
                   ...
                   Label etatcmd=new Label("Etat Commande :");
                     Label prixx=new Label("Prix Total :");
                        ...
                        Button livraison=new Button("Demander Livraison");
                        ...
                        delete.addActionListener(new ActionListener() {
                         ...
                         livraison.addActionListener(new ActionListener() {
                            ...
                            public void actionPerformed(ActionEvent evt) {
                                ...
                                m=commandeService.verifLivraison(a.getId());
                                System.out.println("m :"+m);
                                if(m>1)
                                {
                                
                                Dialog.show("Erreur", "Vous avez déja une livraison pour cette commande", "OK", "Cancel");
                                
                                }
                                else
                                {
                                String ville=Utilisateur.current_user.getVille();
                                String adresse=Utilisateur.current_user.getNomPropriete()+""+Utilisateur.current_user.getRue()+""+Utilisateur.current_user.getVille();
                                System.out.println("id user : " +a.getId_user());
                                System.out.println("id user2 : " +Utilisateur.current_user.getId());
                                Random r = new Random();
                           String val = "" + r.nextInt(10000);

                      System.out.println("val  : " +val);
                          Utilisateur.current_user.toString();
                             TwilioSMS s=new TwilioSMS("AC898b441c0b27885c02934ca85fa6adbc","7b2b83665f17cbca01fe3b1db2dd1265","+16122608997");
                              s.sendSmsAsync("+216"+Utilisateur.current_user.getNumtel(),"Confirmation de votre livraison Monsieur/Madame :"+Utilisateur.current_user.getNom()+" "+Utilisateur.current_user.getPrenom()+"Votre code  de livraison est le suivant :"+val);
                                commandeService.ajouterLivraison(a.getId(),Utilisateur.current_user.getId(),adresse,ville,Utilisateur.current_user,val);
                                Dialog.show("Information", "Votre Livraison vient d'être confirmé ,Vous recevrez un SMS contenant le code de livraison , notre livreur vous contactera dans 3 jours ", "OK", "Cancel");
                                commandeService.changerEtatCmd(a.getId());
                                LivraisonForm t=new LivraisonForm(res);
                                
                            t.show();
                                 // Dialog.show("Information", "OK CMD", "OK", "Cancel");
                                }
